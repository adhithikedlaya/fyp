...
def generate_pink_noise(n, T, alpha, beta, scaling_factor=0.015):
    ...
    f_k = torch.linspace(0, k_max, steps=k_max+1).to(device) * 0.00001 / (2 * np.pi)
    ...
    #C_k=𝐶𝑘𝑒𝑖𝜑𝑘
    C_k_real = C_k * torch.cos(phi_k).to(device)
    C_k_imag = C_k * torch.sin(phi_k).to(device)

    # Halve to preserve mean of 0 and 1/f spectrum?
    C_k_real[1:k_max+1] *= 0.5
    C_k_imag[1:k_max+1] *= 0.5
    C_k_complex_pos = torch.complex(C_k_real, C_k_imag).to(device)
